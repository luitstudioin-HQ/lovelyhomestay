...
import { getExperienceCategories, getExperienceCategoryByHandle } from '@/data/categories'
import { getExperienceListings } from '@/data/listings'
import heroImg from '@/images/hero-img-exp.webp'
import { Metadata } from 'next'
import { redirect } from 'next/navigation'

// Optional catch-all: without this it matches /experience-categories/a/b/c/... to
// any depth and every one of those renders on demand. Prerender the real handles
// (plus the bare route) and 404 everything else.
export const dynamicParams = false

export async function generateStaticParams() {
  const handles = new Set((await getExperienceCategories()).map(({ handle }) => handle))
