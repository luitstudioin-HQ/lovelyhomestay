...
import { getExperienceListingByHandle, getExperienceListings } from '@/data/listings'
...
// Prerender every real handle and 404 the rest, so bots can't mint infinite
// on-demand renders out of this segment.
export const dynamicParams = false

export async function generateStaticParams() {
  const handles = new Set((await getExperienceListings()).map(({ handle }) => handle))
          ...
          {thingsToDo.map((item, index) => (
            <div key={index} className="flex items-start gap-4 sm:items-center sm:gap-8">
              <div className="relative aspect-square w-24 shrink-0 overflow-hidden rounded-xl">
                <Image fill sizes="96px" src={item.imageUrl} alt="" className="object-cover shadow-inner" />
              </div>
