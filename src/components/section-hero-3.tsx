import avatar1 from '@/images/avatars/Image-1.webp'
import avatar2 from '@/images/avatars/Image-2.webp'
  ...
  return (
    ...
    >
      ...
      <div className="relative container flex flex-1 py-22 2xl:py-26">
        <div className={clsx('mx-auto flex w-full max-w-5xl flex-col items-center', showSearchForm ? 'gap-22' : 'justify-center')}>
          {showSearchForm && (
            <FadeIn className="flex w-full" transition={{ delay: 0.1, duration: 1 }}>
              <HeroSearchForm2 initTab={initTab} showTabs={showSearchTabs} />
            </FadeIn>
          )}
          ...
          <div className="flex max-w-4xl flex-col items-center text-center 2xl:max-w-5xl">
            ...
            {description && (
              <FadeIn transition={{ delay: 0.6, duration: 1 }} className="mt-6 max-w-2xl text-base/7 text-white/85 sm:text-lg/8">
                {description}
              </FadeIn>
            )}
            {cta && <FadeIn transition={{ delay: 0.7, duration: 1 }} className="mt-8">{cta}</FadeIn>}
            {showSocialProof && (
              <>
                <FadeIn
