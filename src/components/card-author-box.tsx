...
const CardAuthorBox: FC<CardAuthorBoxProps> = ({ className, author = demo_author }) => {
  ...
  return (
    <Link
      href={`/authors/${handle}`}
      className={`card-author-box group/card relative flex flex-col items-center justify-center rounded-3xl bg-accent px-3 py-5 text-center sm:p-5 ${className}`}
    >
      <Avatar
      ...
      <div className="mt-4 flex w-full items-center justify-between gap-2 rounded-xl bg-background p-2 text-center">
        ...
        <div className="flex-1">
          ...
          <p className="line-clamp-1 text-[10px] font-[450]">years hosting</p>
        </div>
      </div>
    </Link>
  )
