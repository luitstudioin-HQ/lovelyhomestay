...
import avatar2 from '@/images/avatars/Image-1.webp'
import avatar1 from '@/images/avatars/Image-2.webp'
import avatar3 from '@/images/avatars/Image-3.webp'
...
const FeaturedListingCard = ({ stay, priceSuffix, linkCard }: { stay: TStayListing; priceSuffix: string; linkCard: boolean }) => {
      ...
      <div className="absolute inset-x-3 bottom-3 max-w-sm rounded-2xl bg-card p-4 text-card-foreground shadow-lg sm:p-6 lg:bottom-16 lg:-left-16 xl:-left-24">
        ...
        <div className="mt-6 flex items-center justify-between">
          <span>
            ...
            <span className="text-sm font-normal text-neutral-500 dark:text-neutral-400"> {priceSuffix}</span>
          </span>
        </div>
      </div>

      {linkCard && <Link className="absolute inset-0" href={`/stay-listings/${listingHandle}`} />}
    </div>
  )
}
...
const SectionGridFeaturedListings: FC<SectionGridFeaturedListingsProps> = ({
  ...
  eyebrow = 'Most highly rated this month',
  ctaHref = '/stay-categories/santo-domingo',
  ctaLabel = 'Explore more stays',
  ...
  return (
    <div className="relative flex flex-col gap-8 overflow-hidden sm:gap-20 lg:flex-row">
      <div className="flex flex-1 flex-col items-start gap-4 self-center sm:gap-8">
        ...
        <ButtonLargeWithIcon className="mt-8 max-w-lg" href={ctaHref}>
          {ctaLabel}
        </ButtonLargeWithIcon>
      </div>
      ...
      <div className="flex-1 embla sm:overflow-visible" ref={emblaRef}>
        <div className="embla__container w-full">
          {stayListings.map((stay) => (
            <FeaturedListingCard key={stay.id} stay={stay} priceSuffix={priceSuffix} linkCard={linkCards} />
          ))}
        </div>
