import bgGradientImage from '@/images/gradient-bg-1.webp'
...
interface SectionInterestingInforProps {
  ...
  testimonials?: { name: string; job: string; quote: string }[]
  showReviewSource?: boolean
}
...
const STATS = [
  ...
  {
    value: '2B+',
    label: 'Lovely Homestay guest arrivals all-time',
  },
]
...
export default function SectionInterestingInfor({
  ...
  heading = (
    <>
      Some interesting information <span data-slot="italic">about Lovely Homestay!</span>
    </>
  ),
  description = (
    <>
            ...
            <TabPanels>
              {testimonialItems.map((testimonial, index) => (
                <TabPanel key={index} unmount={false} className="flex max-w-md flex-col focus:outline-none">
                  {showReviewSource && <GoogleLogoSvg />}
